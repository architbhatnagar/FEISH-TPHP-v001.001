...
class EthnicitiesController extends AppController {
    ...
    public function add() {
        if ($this->request->is('post')) {
            $ethnicity = $this->request->data;
            foreach ($ethnicity['Ethnicity'] as $value) {
                $saveEthnicity['Ethnicity'] = $value;
                $this->Ethnicity->create();
                if ($this->Ethnicity->save($saveEthnicity)) {
                    $flag = true;
                } else {
                    $flag = false;
                }
            ...
            if($flag){
                $this->Session->setFlash(__('The Ethnicity has been saved.'),'success');
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The Ethnicity could not be saved. Please, try again.'),'error');
            }
        }
    }
